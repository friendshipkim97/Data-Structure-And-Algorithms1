...
public class Polynom {
	week9.MyLinkedList<Term> myP; //ArrayList 자바가 기본적으로 제공해주는클래스, 항의숫자는 자동으로 세주는메소드를 이용한다. 
    String polyName;
...
//		return myP.indexOf(t);
		...
		for(int i=0; i<myP.size(); i++) 
			if(t.equals(myP.get(i))) index = myP.indexOf(myP.get(i)); //indexOf메소드는 이미 자바에서 정해놓은메소드, 따라서 오브젝트타입으로 받기 때문에 해쉬코드를 비교한다. 제대로넣기위해선 Term타입으로 넣고 비교해야함
    ...
    public void printPoly() {
    	...
    	for(int i=0; i<myP.size(); i++) {
    		...
    		System.out.print(temp+ myP.get(i).toString()); //자바의 모든것의 상위클래스 object클래스 
